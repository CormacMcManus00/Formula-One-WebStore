function copyNumber(){
    var copyNum = document.getElementById('contactNo');
    copyNum.select();
    
    document.execCommand("copy");
    alert("copied text: " + copyNum)
}

if (localStorage.getItem('productVal') == null ) {
    localStorage.setItem('productVal', 0);
}

window.onload = function EmbedCheckoutDetails(){
        var buybutton = document.getElementById('buy-button');
        if (localStorage.getItem('loggedIn') != 1){
            buybutton.setAttribute("class", "btn btn-light");
            buybutton.setAttribute("type", "button");
            document.getElementById("no-login-failure").setAttribute("class","alert alert-danger");
        }
    
    
        document.getElementById('checkout-details').innerHTML = ( (localStorage.getItem('checkout')) + " items valued at €" + (localStorage.getItem('productVal')));
}
...
function AddToCart(price){
    totalPrice = parseInt(localStorage.getItem('productVal'));
    totalPrice = totalPrice + parseInt(price);
    localStorage.setItem('productVal',(totalPrice));
}  
...
$(document).ready(function(){
    ...
    $('form[name="paymentdetails"]' ).submit(function( event ) {
        ...
        if (cardnumber=="1234 5678 9102 3456") {
            ...
            localStorage.setItem('productVal',total);
            // makes sure that when we goto another page - the total help in localstorage is zero
            $( "#checkout" ).html(total);
